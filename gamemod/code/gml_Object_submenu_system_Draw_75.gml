...
menu_string += txt("Display Timer on Screen: ");
if (ds_zero_options("TimerOnScreen") == 0)
{
    menu_string += (txt("[wave]<[/wave] No [wave]>[/wave]") + "\n");
}
else if (ds_zero_options("TimerOnScreen") == 1)
{
    menu_string += (txt("[wave]<[/wave] Game Timer [wave]>[/wave]") + "\n");
}
else if (ds_zero_options("TimerOnScreen") == 2)
{
    menu_string += (txt("[wave]<[/wave] Real Timer [wave]>[/wave]") + "\n");
}
else if (ds_zero_options("TimerOnScreen") == 3)
{
    menu_string += (txt("[wave]<[/wave] Both [wave]>[/wave]") + "\n");
}
if (selection == 17)
{
    menu_string += cursor_helmet();
...
menu_string += "Confine Mouse: ";
if (ds_zero_options("Mouse Capture"))
{
    menu_string += (txt("[wave]<[/wave] ON [wave]>[/wave]") + "\n");
}
else
...
var opt_y_off = 0;
opt_y_off = ((selection > 8.5) * global._screen_height) / 3;
options_y_off = lerp(options_y_off, opt_y_off, 0.2);
draw_text_scribble(48, (global._screen_height + menu_offset) - options_y_off, menu_string);
