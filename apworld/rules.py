...
CAN_GRAPPLE = OptionFilter(ProgressiveGrappleBeam, True) & Has("Progressive Grapple Beam", 2) | OptionFilter(ProgressiveGrappleBeam, False) & Has("Grapple Beam")
CAN_SPACEJUMP = OptionFilter(ProgressiveGrappleBeam, True) & Has("Progressive Grapple Beam", 1) | OptionFilter(ProgressiveGrappleBeam, False) & Has("Space Jump Boots")
CAN_TRIGGER_BOMB_SLOTS = CAN_BOMB | CAN_PB
CAN_KILL_STONE_TOADS = CAN_BOMB | CAN_PB
...
def set_tallon_location_rules(world: MetroidPrimeOriginsWorld):
    ...
    # West Tallon
    set_location_rule("(Tallon Overworld) Arbor Chamber", Has(PLASMA) & CAN_TRAVERSE_LOW_OVERHANG_WITH_SMWJ, world)

    # Crash Site Left
    set_location_rule("(Tallon Overworld) Crash Site - Underwater Ledge", (Has(GRAVITY) & CAN_TRAVERSE_LOW_OVERHANG_WITH_SMWJ) | CAN_GRAPPLE, world)
...
def set_chozo_ruins_location_rules(world: MetroidPrimeOriginsWorld):
    ...
    set_location_rule("(Chozo Ruins) Dynamo - Top", CAN_TRAVERSE_LOW_OVERHANG_WITH_SMWJ, world)
    ...
    # Upper Furnace
    set_location_rule("(Chozo Ruins) Furnace - Boost Ceiling", CAN_BOOST & (CAN_SPIDER | CAN_GRAPPLE) & CAN_DESTROY_BOMB_BLOCKS & CAN_PB, world)
...
def set_magmoor_location_rules(world: MetroidPrimeOriginsWorld):
    ...
    set_location_rule("(Magmoor Caverns) Warrior Shrine - Statue", Has(VARIA) & ((CAN_BOOST & CAN_TRAVERSE_LOW_OVERHANG_WITH_SMWJ) | CAN_IBJ | CAN_GRAPPLE), world)
    set_location_rule("(Magmoor Caverns) Fiery Shores - from Warrior Shrine", CAN_PB & Has(MORPH) & CanReachLocation("(Magmoor Caverns) Warrior Shrine - Statue"), world)
    ...
    # Geothermal Core
    set_location_rule("(Magmoor Caverns) Plasma Processing", Has(MORPH) & Has(ICE) & Has(VARIA) & CAN_TRAVERSE_LOW_OVERHANG_WITH_SMWJ & CAN_BOOST & CAN_DESTROY_BLOCKS_WHILE_MORPHED & CAN_GRAPPLE, world)

    # West Magmoor
    set_location_rule("(Magmoor Caverns) Magmoor Workstation", CAN_TRIGGER_BOMB_SLOTS & CAN_SPIDER & Has(WAVE), world)
...
def set_phendrana_location_rules(world: MetroidPrimeOriginsWorld):
    ...
    # Central Phendrana
    set_location_rule("(Phendrana Drifts) Ruined Courtyard", (Has(MORPH) & CAN_TRAVERSE_LOW_OVERHANG_WITH_SMWJ) | (CAN_BOOST & CAN_TRIGGER_BOMB_SLOTS), world)

    # Observatory
    set_location_rule("(Phendrana Drifts) Observatory", CAN_BOOST & CAN_TRIGGER_BOMB_SLOTS, world)

    # Thardus Area
    set_location_rule("(Phendrana Drifts) Quarantine Cave - Thardus Reward", CAN_TRAVERSE_LOW_OVERHANG, world)
    set_location_rule("(Phendrana Drifts) Quarantine Monitor", Has(MORPH) & (CAN_GRAPPLE | CAN_IBJ), world)

    # Research Lab Hydra
    set_location_rule("(Phendrana Drifts) Research Lab Hydra", CAN_SUPER_MISSILE, world)

    # Research Lab Aether
    set_location_rule("(Phendrana Drifts) Control Tower", Has(MISSILE) & CAN_DESTROY_GLASS_BLOCK & CAN_TRAVERSE_LOW_OVERHANG_WITH_SMWJ, world)
    ...
    # Frost Cave
    set_location_rule("(Phendrana Drifts) Frost Cave", Has(MISSILE) & (CAN_SPACEJUMP | CAN_IBJ | CAN_GRAPPLE), world)

    # True Edge
    set_location_rule("(Phendrana Drifts) Security Cave", CAN_GRAPPLE & Has(MORPH), world)
    set_location_rule("(Phendrana Drifts) Storage Cave", CAN_PB, world)

    # Hunter Cave
    set_location_rule("(Phendrana Drifts) Gravity Chamber - Spike Tunnel", CAN_GRAPPLE, world)
    set_location_rule("(Phendrana Drifts) Gravity Chamber", CAN_TRAVERSE_UNDERWATER | Has(MISSILE) | CAN_SPACEJUMP, world)
...
def set_region_connection_rules(world: MetroidPrimeOriginsWorld):
    ...
    set_entrance_rule("Crash Site Left to Right", CAN_GRAPPLE | (Has(GRAVITY) & Has(MORPH)) | CAN_IBJ | CAN_SPACEJUMP, world)
    ...
    set_entrance_rule("Energy Core to Furnace", CAN_TRAVERSE_LOW_OVERHANG | CAN_TRIGGER_BOMB_SLOTS, world)
    set_entrance_rule("Energy Core to Gathering Hall", Has(MORPH), world)

    set_entrance_rule("Furnace to Upper Furnace", Has(MORPH) & CAN_DESTROY_BOMB_BLOCKS & (CAN_SPIDER | CAN_GRAPPLE | SM_WALLJUMP), world)
    ...
    set_entrance_rule("Elder Hall to Lower Reflecting Pool", Has(WAVE) & CAN_BOMB & CAN_TRAVERSE_LOW_OVERHANG_WITH_SMWJ, world)

    set_entrance_rule("Lower Pool to Upper Pool", (CAN_DESTROY_BOMB_BLOCKS & CAN_BOOST) | CAN_GRAPPLE | CAN_IBJ, world)

    set_entrance_rule("Upper Reflecting Pool to Overgrown Cavern", Has(MISSILE) & CAN_KILL_STONE_TOADS, world)
    set_entrance_rule("Upper Reflecting Pool to East Tallon", CAN_KILL_STONE_TOADS & Has(ICE), world)
    ...
    set_entrance_rule("Colored Blocks to Phazon Entrance", CAN_GRAPPLE & CAN_PB & CAN_BOMB & Has(MORPH), world)

    set_entrance_rule("Elite Control Access to Ventilation Shaft", Has(WAVE) & Has(ICE) & (CAN_SPACEJUMP | CAN_GRAPPLE | CAN_IBJ), world)
    ...
    set_entrance_rule("Fungal Hall Access to Phazon Mining Tunnel", CAN_GRAPPLE & CAN_PB & Has(ICE), world)

    set_entrance_rule("Phazon Mining Tunnel to Fungal Hall B", Has(MORPH) & CAN_DESTROY_BLOCKS_WHILE_MORPHED & (CAN_GRAPPLE | Has(PHAZON)), world)

    set_entrance_rule("Fungal Hall B to Metroid Quarantine B", Has(MORPH) & CAN_DESTROY_BOMB_BLOCKS & CAN_GRAPPLE & Has(PLASMA), world)

    set_entrance_rule("Metroid Quarantine B to Omega Pirate Area", Has(PLASMA), world)

    set_entrance_rule("Omega Pirate to Processing Center Access", Has(PHAZON) & (CAN_GRAPPLE | CAN_IBJ | CAN_SPACEJUMP), world)
    ...
    set_entrance_rule("Processing Center to West Elevator", CAN_TRAVERSE_LOW_OVERHANG_WITH_SMWJ & Has(MORPH) & Has(ICE), world)
    set_entrance_rule("Processing Center to Elite Control Access", CAN_TRAVERSE_LOW_OVERHANG_WITH_SMWJ & CAN_PB & Has(MORPH), world)
